		...
		if ( ! empty( $notices ) ) {
			...
			foreach ( $notices as $notice ) {
				...
				if ( $notice['dismiss_class'] ) {
					$dismiss_url = wp_nonce_url( add_query_arg( 'dismiss_wc_nyp_notice', $notice['dismiss_class'] ), 'wc_nyp_dismiss_notice_nonce', '_wc_nyp_admin_nonce' );
					echo '<a class="wc-mnm-dismiss-notice notice-dismiss" href="' . esc_url( $dismiss_url ) . '">' . esc_html__( 'Dismiss', 'wc_name_your_price' ) . '</a>';
				}
